...
public class ReviewSupplierRepositoryImpl extends GenericRepositoryImpl<ReviewSupplier> implements ReviewSupplierRepository {
    ...
    @Override
    public List<Predicate> doFilter(Map<String, String> params, CriteriaBuilder b, Root<ReviewSupplier> root) {
        ...
        if (kw2 != null && !kw2.isEmpty()) {
            predicates.add(b.like(root.get("receiptImportId").get("supplierId").get("name").as(String.class), "%" + kw2 + "%"));
        }
        return predicates;
    }
